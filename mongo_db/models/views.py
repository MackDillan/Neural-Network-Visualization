...
import uuid
import json
from .db import models as monngo_models
...
class ModelsView(APIView):
    def get(self, request, pk=None):
        if pk:
            ...
            result = list(monngo_models.find({"_id": pk}, {"_id": 1, "version": 1, "json_data": 1, "created": 1, "updated": 1}))
            if not result:
                return Response({"error": "Document not found"}, status=status.HTTP_404_NOT_FOUND)
            return Response(result, status=status.HTTP_200_OK)
        else:
            # Retrieve the latest version of all documents
            result = list(monngo_models.aggregate([
                {"$sort": {"version": -1}},
    ...
    def post(self, request):
        ...
        try:
            ...
            monngo_models.insert_one(document)
    ...
    def put(self, request, pk):
        ...
        try:
            ...
            last_doc = monngo_models.find_one({"_id": pk}, sort=[("version", -1)])
            if not last_doc:
                return Response({"error": "Document not found"}, status=status.HTTP_404_NOT_FOUND)
            ...
            monngo_models.update_one({"_id": pk}, {"$set": document})
    ...
    def delete(self, request, pk):
        ...
        result = monngo_models.delete_many({"_id": pk})
...
class JSONDataDownloadView(APIView):
    def get(self, request, pk):
        try:
            ...
            document = monngo_models.find_one({"_id": pk})
            if not document:
                return Response({"error": "Document not found"}, status=status.HTTP_404_NOT_FOUND)
